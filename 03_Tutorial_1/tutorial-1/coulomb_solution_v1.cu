 ...
 *               n.b.2 the idea here is to redo the calculation but
 *                     demote all variables and operations to float
 * compilation:  nvcc ./coulomb_solution_v1.cu
 * usage:        ./a.out  
 * result:       coulomb sum: -230.611389160 (FP32)
 *               2bcompared2
 *               coulomb sum: -230.611048407 (FP64)
 */
...
__global__ void coulomb(int NN, float *x, float *y, float *z, float *q, float *cp)
{
    int i, j;
    float pcs, rx, ry, rz, r;
...
int main()
{
  ...
  */
  ...
  cudaMallocManaged(&x, n * NTB * sizeof(float));
  cudaMallocManaged(&y, n * NTB * sizeof(float));
  cudaMallocManaged(&z, n * NTB * sizeof(float));
  cudaMallocManaged(&q, n * NTB * sizeof(float));
  cudaMallocManaged(&cp, n * NTB * sizeof(float));
  ...
  */
  fp1 = fopen("MOLPDB", "r");
  for (i = 0; i < N; i++) {
      fscanf(fp1, "%*s%*d%*s%*s%*d%f%f%f%f%*f%*f%*s\n", &x[i], &y[i], &z[i], &q[i]);
      // printf("%6d%8.3f%8.3f%8.3f%10.5f\n", i, x[i], y[i], z[i], q[i]);
  }
  fclose(fp1);
  ...
  */
  for (i = N; i < n * NTB; i++) {
      x[i] = (float) rand() / (float) i;
      y[i] = (float) rand() / (float) i;
      z[i] = (float) rand() / (float) i;
      q[i] = (float) 0;
  }
