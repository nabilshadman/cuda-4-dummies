 ...
 *                    however, using predominantly local variables;
 ...
 *               n.b.5 here we want to actually set up x,y coordinates
 *                     locally inside the kernel and communicate back
 *                     and forth data as limited as possible;
 *                     
 * compilation:  nvcc ./pi_solution_v4.cu 
 * usage:        /usr/bin/time -v ./a.out  
 *               nsys nvprof ./a.out  
 * result:       computed pi:     3.146184824
 *               machine  pi:     3.141592654
 *               timings: 0.09user 5.29system 0:05.52elapsed 97%CPU 
 *                        2bcompared2
 *                        1.61user 5.67system 0:07.43elapsed 98%CPU based on ./pi_solution_v3.cu
 *               kernel exe time: ~0.0078 s  
 *                                2bcompared2 
 *                                ~1.5123 s based on ./pi_solution_v3.cu
 *               
 */
...
__global__ void distances4(int *s)
{
    int i;
    double x, y, r;
    curandState state;
...
int main()
{
  ...
  double ratio_partial_areas, computed_pi;


 /*
  * allocate memory for the only remainin array, s[], being 
  * transmitted forth and back into kernel code again using 
  * CUDA-managed unified memory
  * n.b. if N is not an integral multiple of NTB, the number of 
  *      threads in the threadblock, we shall consider that only
  *      in the final threadblock, where a certain fraction may
  *      then have to be excluded from the evaluation, hence 
  ...
  * n.b.4 moreover, there are no x[], y[], r[] arrays to
  *       be sent back and forth now, just the final 
  *       array, s[], which already contains partial 
  *       results computed at the level of threadblocks
  */
  distances4<<<n, NTB>>>(&s[0]);
  cudaDeviceSynchronize(); 

 /*
  * count all cases of r > 1.0 because these are points 
